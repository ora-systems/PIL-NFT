...
var ghost = 0.5;
  ...
  for (var t = 0; t < 1.05; t += 0.001) {
      ...
      ghost = 0.5;
    var roo = 0.1;
    var distance = min(min(abs(ghost-t), abs(ghost-(t+1.0))), abs((ghost+1)-t));
    if (t > ghost) ghost++;
    var range = (1.0-distance/roo);
    if (range < 0) range = 0;
      
      var x = radius*0.725*pow(sin(t*2*PI),3), y = -5 - radius*1.1*(13*cos(t*2*PI) - 5*cos(4*t*PI) - 2*cos(6*t*PI) - cos(8*t*PI))/16, z = 5 + 20*cos(t*PI*6);
